				...
				if (clList.getLength() != 0) {
					...
					for (int cLtemp = 0; cLtemp < clList.getLength(); cLtemp++) {
						...
						if (clNode.getNodeType() == Node.ELEMENT_NODE) {
							...
							if (!(natureOfInterest[cLtemp][i].equalsIgnoreCase("LP"))
									|| !(natureOfInterest[cLtemp][i].equalsIgnoreCase("MG"))) {
								...
								System.out.print(" "
										+ xPath.compile("//Contract/Eff_Date").evaluate(xmlDocument).replace("-", "")
												.substring(0, 8)
